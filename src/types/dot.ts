...
class Dot implements StarType {
  ...
  update(speedMultiplier: number = 1) {
    ...
    for (let i = 0; i < this.stars.length; i++) {
      ...
      if (
        star.x > this.canvas.width + this.overflowSize ||
        ...
        // If the direction is top-right
          ...
          y =
            x == startX
              ? randomNumber(startX, this.canvas.height - this.overflowSize)
  ...
  randomSize() {
    return typeof this.config.starSize == 'object'
      ? randomNumber(this.config.starSize[0], this.config.starSize[1])
      : this.config.starSize
  }
  randomOpacity() {
    let opacity = this.config.randomOpacity
    if (typeof opacity == 'boolean') return !opacity ? 1 : (opacity ? Math.random() : 1).toFixed(2)
    else return (Math.random() * (opacity[1] - opacity[0]) + opacity[0]).toFixed(2)
  }
  randomSpeed() {
